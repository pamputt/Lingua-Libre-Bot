...
class KuWiktionary(Wiktionary):
    ...
    def execute(self, record):
        ...
        if not is_already_present:
            ...
            print(f"--- END BEFORE --->{transcription}")
        ...
        if not pronunciation_section:
            print("PAS DE SECTION PRON")
            
        # Remove blank lines in the pronunciation section
        pronunciation_section = self.clean_section(pronunciation_section)
        print(f"--- PRON SECTION (clean) ---\n{pronunciation_section}\n--- END PRON SECTION (clean) ---")
    ...
    def safe_append_text(self, content, text):
        ...
        print(f"DEBUT: {content[:index]}")
        print(f"FIN: {content[index:]}")
