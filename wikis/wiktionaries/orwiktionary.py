...
PRONUNCIATION_LINE = "\n* {{ଅଡ଼ିଓ|$1|$3|lang=$2}}"
#{{audio|Or-ଓଡ଼ିଆ 01.oga|ଧ୍ୱନି|lang=or}}
...
class OrWiktionary(Wiktionary):
    ...
    # Prepare the records to be added on the Oriya Wiktionary:
    # - Fetch the needed language code map (Qid -> BCP 47, used by orwiktionary)
    # - Get the labels of the speaker's location in Oriya
    ...
    # Try to use the given record on the Oriya Wiktionary
    def execute(self, record):
        # Normalize the record using orwiktionary's titles conventions
        ...
        if is_already_present:
            print(record["id"] + "//" + transcription + ": already on orwiktionary")
            return False
    ...
    # Normalize the transcription to fit orwiktionary's title conventions
    ...
    def get_pronunciation_section(self, wikicode):
        for section in wikicode.sections:
            ...
            if section.title.replace(" ", "").lower() == "==ଉଚ୍ଚାରଣ==":
                return section
